...
{
    public class ZohoUprnService : IZohoUprnService
    {
        ...
        private const string UprnModule = "Premises";

        public ZohoUprnService(IRoxusLoggingRepository roxusRepository, IZohoAuthService zohoAuthService)
        {
        ...
        public async Task<ApiResultDto<UprnResponse>> GetUprnById(string apiKey, string uprnId)
        {
            ApiLogging apiLogging = null;
            AppConfiguration appConfig = null;
            string endpoint = string.Empty;
            var apiResult = new ApiResultDto<UprnResponse>();
            try
            {
                ...
                endpoint = $"{appConfig.EndPoint}/{UprnModule}/{uprnId}";
                           ...
                           HttpCompletionOption.ResponseHeadersRead))
                {
                    ...
                    if (response.StatusCode == HttpStatusCode.OK)
                    {
                        ...
                        var responseObj = JsonConvert.DeserializeObject<UprnResponse>(json["data"][0].ToString());
        ...
        public async Task<ApiResultDto<UpsertResponse<LinkingResponse>>> UPRN_LinkingWithTitles(string apiKey, string uprnId, string titleIds)
        {
            ApiLogging apiLogging = null;
            AppConfiguration appConfig = null;
            string endpoint = string.Empty;
            ...
            try
            {
                ...
                endpoint = $"{appConfig.EndPoint}/{UprnModule}";

                // Handle Object for Linking
                var upsertRequest = new UpsertRequest<UPRN_LinkingTitles>();
                var uprnLinkingTitles = new UPRN_LinkingTitles();
                uprnLinkingTitles.id = uprnId;
                string[] titles = titleIds.Split(",");
                foreach (var title in titles)
                {
                    var relatedTitle = new UPRN_RelatedTitles()
                    {
                        Title = title
                    };
                    uprnLinkingTitles.UPRN_Titles.Add(relatedTitle);
                }
                upsertRequest.data.Add(uprnLinkingTitles);
                           ...
                           HttpCompletionOption.ResponseHeadersRead))
                {
                    ...
                    apiLogging = new ApiLogging()
                    {
                        ...
                        CreatedDate = DateTimeHelpers.ConvertDateTimeToString(DateTime.UtcNow),
                        HttpMethod = "PUT",
                        ApiName = ZohoConstants.ZCRM_UPRN_LINKING_WITH_TITLE,
                        Endpoint = endpoint
                    };
                    return apiResult;
                }
